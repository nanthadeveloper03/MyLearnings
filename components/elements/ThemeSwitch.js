...
export default function ThemeSwitch() {
    ...
    const [togglETHeme, setTogglETHeme] = useState(
        () => localStorage.getItem("togglETHeme") || "is_light"
    )

    const { isTheme } = useSelector((state) => state.auth);

    const switchTheme = () => {
        let theme = (isTheme === "is_light") ? 'is_dark' : 'is_light';
    ...
    return (
        <>
            <div className="mode-switcher cursor-pointer" onClick={switchTheme}>

                {isTheme == 'is_dark' &&
                    <svg width={16} height={16} viewBox="0 0 16 16" fill="#0000" xmlns="http://www.w3.org/2000/svg">
