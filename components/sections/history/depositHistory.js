...
const WithdrawTable = ({ randomValue }) => {
    ...
    const columns = [
...

        {
            ...
            cell: row => (<>
                ...
                <Tooltip title="Copy Transaction Id"><FileCopyIcon color='warning' className="cursor-pointer" onClick={() => copyText(row.txid, 1)} /></Tooltip>
            </>),
            sortable: true,
            wrap: true,
            grow: 2,
    ...
    const history = async (currentPage, perPage) => {
        ...
        try {
            setIsLoading(true)
            let response = await apiRequest('/historyOfDeposit', { 'currentPage': currentPage, 'perPage': perPage, filter: filter })
            ...
            if (response?.status) {
                ...
                console.log(response.data.totalPages,'response.data.total');
