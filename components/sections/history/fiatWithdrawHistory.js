...
const FiatDepositTable = ({ randomValue }) => {
    ...
    const columns = [
        ...
        {
            name: 'Bank',
            selector: row => row.userBank[0]?.bankName,
            sortable: true,
            grow: 3,
        },
          {
    ...
    const history = async (currentPage, perPage) => {
        ...
        try {
            setIsLoading(true)
            let response = await apiRequest('/fiatWithdrawHistory', { 'currentPage': currentPage, 'perPage': perPage, filter: filter })
console.log(response.data?.historyDetails,'response.data?.historyDetails');
