...
import SliderWithStep from '@/components/sections/newTrade/SliderWithStep';
...
const BuySell = ({pairInfo,pairBalancefrom}) => {
  ...
  const [pricebuyMarket,setpricebuyMarket] = useState(pairInfo.lastPrice);
  const [totalbuyMarket,settotalbuyMarket] = useState("");

  const [amountsellMarket,setamountsellMarket] = useState("");
  const [pricesellMarket,setpricesellMarket] = useState(pairInfo.lastPrice);
