...
export default function withdrawConfirm({ showPopup, formData, onSubmitWithdraw }) {
    ...
    const sendOtp = async () => {
        ...
        setValue('otp', '')
        try {
            const response = await apiRequest('/fiatWithdraw/sendotp', { type: 'sendOTP' });
    ...
    const onSubmit = async (data) => {
        try {
            ...
            let requestData = {
                ...
                userAccountNumber: formData.accountNumber,
                withdrawOtp:Number(data.otp),
            }
            console.log(requestData,'formDataformData');
            const response = await apiRequest('/fiatWithdraw', requestData)
            if (response?.status) {
                onSubmitWithdraw()
                closeModal()
            }
                            ...
                            <>
                                ...
                                <div className="cm_lisw mb-4">
                                    ...
                                    <div className="d-flex justify-content-between lisw_rw mb-3">
                                        <span className="lisw_lf">Withdraw To Bank Account</span>
                                        <span className="lisw_rig">{formData && formData.accountNumber} {formData && formData.accountNumber && <FileCopyIcon color='warning' className="cursor-pointer" onClick={() => copyText(formData.accountNumber)} />} </span>
                                    </div>

                                    
                                    {formData && formData.bankName &&
                                        <div className="d-flex justify-content-between lisw_rw mb-3">
                                            <span className="lisw_lf">Withdraw Bank Name</span>
                                            <span className="lisw_rig fw600">{formData && formData.bankName || '---'}</span>
                                        </div>}
                                    {formData && formData.accountHolderName &&
                                        <div className="d-flex justify-content-between lisw_rw mb-3">
                                            <span className="lisw_lf">Withdraw Account Holder Name</span>
                                            <span className="lisw_rig fw600">{formData && formData.accountHolderName || '---'}</span>
                                        </div>}

                                    {formData && formData.accountType &&
                                        <div className="d-flex justify-content-between lisw_rw mb-3">
                                            <span className="lisw_lf">Account Type</span>
                                            <span className="lisw_rig fw600">{formData && formData.accountType || '---'}</span>
                                        </div>}

                                    <div className="d-flex justify-content-between lisw_rw mb-3">
                                        <span className="lisw_lf">Withdraw Amount</span>
                                        <span className="lisw_rig fw600">{formData && formatNumber(formData.amount, formData.decimalPoint) || '---'} {formData && formData.currency}</span>
                                    </div>

                                    <div className="d-flex justify-content-between lisw_rw mb-3">
                                        <span className="lisw_lf">Withdraw Fee</span>
                                        <span className="lisw_rig fw600">{formData && formatNumber(formData.feeAmount, formData.decimalPoint) || '---'} {formData && formData.currency}</span>
                                    </div>

                                    <div className="d-flex justify-content-between lisw_rw mb-3">
                                        <span className="lisw_lf">Receive Amount</span>
                                        <span className="lisw_rig fw600">{formData && formatNumber(Number(formData.amount)-formData.feeAmount, formData.decimalPoint) || '---'} {formData && formData.currency}</span>
                                    </div>
