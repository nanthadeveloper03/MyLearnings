...
export default function balanceCard() {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                <>
                    <Image src={row.image || ''} height={30} width={30} className="mx-3 rounded-circle" alt={row.symbol} />
                    {row.name}
                </>
            ),
            sortable: true,
        ...
        },
        {
            ...
            cell: row => (
                <>
                    <ul className="d-flex flex-nowrap act_lis1 gap-4">
                        <li>
                            <Link href={`/deposit/?coin=${row.symbol}`} className={`pri_color ${(row.depositStatus == 1) ? '' : 'disabled-icon'}`} title={row.depositStatus ? 'Make Deposit' : ' Deposit Disabled'}>
                                Deposit
                            </Link>
                        </li>
                        <li>
                            <div className={`pri_color ${(row.withdrawStatus == 1) ? 'cursor-pointer' : 'disabled-icon'}`} title={row.withdrawStatus ? 'Make Withdraw' : ' Withdraw Disabled'} onClick={() => { redirectPage(1, row.symbol) }}>
                                Withdraw
                            </div>
                        </li>
                        <li>
                            <Link href="/soon_convert" className="pri_color" title="Coming Soon">
                                Convert
                            </Link>
                        </li>
                    </ul>
        ...
        <div className="wallet_listtable">
            <div style={{ height: "100%", width: "100%" }}>
                <div className="row wallet_table_head">
                    <h3>Spot Assets</h3>
                    <div className="input-group search-area flex-nowrap">
                        <input
                            type="text"
