...
class CustomDatafeed {
    ...
    constructor(pairInfo) {
        ...
        this.uprousdtpricechange=pairInfo.uprousdtpricechange;
    }
    ...
    async getBars(symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) {
        ...
        url = `https://api.binance.com/api/v3/klines?symbol=ONEUSDT&interval=${interval}&startTime=${1728311821 * 1000}&endTime=${to * 1000}`;
        ...
        try {
            ...
            let increase = 0;
            if(symbolInfo.name.includes("UPRO") == true){                  
                increase = this.uprousdtpricechange;
            }
            else{
            ...
            const bars = datacustom.map(item => ({
                time: item[0],
                low: parseFloat(item[3]) + parseFloat(increase),
                high: parseFloat(item[2]) + parseFloat(increase),
                open: parseFloat(item[1]) + parseFloat(increase),
                close: parseFloat(item[4]) + parseFloat(increase),
    ...
    subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback) {
        ...
        ws.onmessage = (event) => {
            ...
            if (message.e === 'kline') {
                ...
                const bar = {
                    time: kline.t,
                    low: parseFloat(kline.l+2),
                    high: parseFloat(kline.h+2),
                    open: parseFloat(kline.o+2),
                    close: parseFloat(kline.c+2),
                    volume: parseFloat(kline.v+2),
                };
