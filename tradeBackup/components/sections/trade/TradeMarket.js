...
const TradeMarket = ({ pairInfo }) => {
  ...
  if (pairInfo?.pair?.includes("UPRO") == false) {
    ...
    taborder = 1;
    increase = pairInfo.uprousdtpricechange;
  }
                ...
                <div className='trd_table1'>
                  ...
                  <div className="virtual_tbody anim_scoll1 d-flex flex-column w-100">
                    {askData.length > 0 && askData.slice(0, 7).reverse().map((ask, index) => {
                      let price = decimalRoundOff((parseFloat(ask.price)+parseFloat(increase) * inrPrice), priceDecimal);
                      let amount = ask.quantity
                      let total = decimalRoundOff((parseFloat(price) * amount), priceDecimal)
                    ...
                    <div className="virt_tinrw d-flex align-items-center w-100">
                      <div className="vir_fd3">
                        {orderBookData.ticker &&
                          <h4 className={`${orderBookData.ticker.perClr}`}>{decimalRoundOff((parseFloat(orderBookData.ticker.price)+parseFloat(increase) * inrPrice), priceDecimal)}</h4>
                        }
                  ...
                  <div className="virtual_tbody anim_scoll1 d-flex flex-column w-100">
                    {bidData.length > 0 && bidData.slice(0, 7).map((bid, index) => {
                      let price = decimalRoundOff((parseFloat(bid.price)+parseFloat(increase) * inrPrice), priceDecimal);
