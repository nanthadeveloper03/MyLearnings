...
export default function RecentTrade({ deviceType, pair, pairInfo }) {
    ...
    const [increase, setincrease] = useState(pairInfo.uprousdtpricechange);
                ...
                <div className="virtual_tbody d-flex flex-column w-100 mt-2">
                    {trades && trades.length > 0 ?
                        ...
                        trades.map((trade, index) => {
                            let price = (pairInfo.fromCurrency == "UPRO") ? decimalRoundOff(((parseFloat(trade.p) + parseFloat(increase)) * inrPrice), priceDecimal) : decimalRoundOff((trade.p * inrPrice), priceDecimal);
                            let amount = trade.q
                            return (
